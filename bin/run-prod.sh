...
DEPLOY_JAVA_OPTS="-Dspring.profiles.active=prod"
...
deploy_api_name="eGovFrameStatMng-api"
...
header "kill eGovFrameStatMng api"
deploy_pid=`ps -ef | grep java | grep ${deploy_api_name} | awk '{ print $2 }'`

if [ -z $deploy_pid ]
then
    echo " no eGovFrameStatMng api process"
else
...
header "run eGovFrameStatMng api"
...
deploy_pid=`ps -ef | grep java | grep ${deploy_api_name} | awk '{ print $2 }'`
echo "run eGovFrameStatMng api [${deploy_pid}]"
